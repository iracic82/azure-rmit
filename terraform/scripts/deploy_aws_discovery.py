...
class InfobloxSession:
    ...
    def create_aws_key(self):
        access_key_id = os.getenv(
            "INSTRUQT_AWS_ACCOUNT_INFOBLOX_DEMO_AWS_ACCESS_KEY_ID"
        )
        secret_access_key = os.getenv(
            "INSTRUQT_AWS_ACCOUNT_INFOBLOX_DEMO_AWS_SECRET_ACCESS_KEY"
        )
        ...
        if response.status_code == 409:
            print("AWS key already exists, skipping creation.")
        else:
            response.raise_for_status()
    ...
    def fetch_cloud_credential_id(self):
        ...
        while waited < timeout:
            try:
                ...
                if response.status_code == 403:
                    print("403 Forbidden - likely propagation delay")
                response.raise_for_status()
                ...
                for cred in creds:
                    if cred.get("credential_type") == "Amazon Web Services":
                        ...
                        print(f"AWS Cloud Credential ID found: {credential_id}")
        ...
        raise RuntimeError(
            "Timed out after 2 minutes waiting for AWS Cloud Credential."
        )
    ...
    def inject_variables_into_payload(
        self, template_file, output_file, dns_view_id, cloud_credential_id, account_id
    ):
